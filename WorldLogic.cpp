...
WorldLogicPrepare WorldLogic::Prepare() { return {*this}; }

bool WorldLogic::AllCitiesUnique() const {
    for (int i = 0; i < GetPlayersCount(); ++i) {
        auto& player = GetPlayer(i);
        if (player.GetCity() == city_unknown) return false;
        for (int j = i + 1; j < GetPlayersCount(); ++j) {
            auto& another = GetPlayer(j);
            if (player.GetCity() == another.GetCity()) return false;
        }
        return true;
    }
    return false;
...
bool WorldLogic::AllColorsUnique() const {
    for (int i = 0; i < GetPlayersCount(); ++i) {
        auto& player = GetPlayer(i);
        if (player.GetColor() == player_color_unknown) return false;
        for (int j = i + 1; j < GetPlayersCount(); ++j) {
            auto& another = GetPlayer(j);
            if (player.GetColor() == another.GetColor()) return false;
        }
        return true;
    }
    return false;
...
PlayerLogic& WorldLogic::GetPlayer(int i) { return _players[i]; }

const PlayerLogic& WorldLogic::GetPlayer(int i) const { return _players[i]; }
...
Players WorldLogic::GetPlayers() { return ::Players{*this}; }

ConstPlayers WorldLogic::GetPlayers() const { return ::ConstPlayers{*this}; }

AllArmies WorldLogic::Armies() { return AllArmies{*this}; }
...
bool WorldLogic::HasPoliciesInDeck() const { return !_policyDeck.Empty(); }
...
void WorldLogic::CompleteDraft() {
    GetPlayers().ForEach([](PlayerLogic& player) { player.EndDraft(); });
...
ConstPlayers::ConstPlayers(const WorldLogic& world) : _world{world} {}
...
bool WorldLogic::DraftComplete() const { return _draftComplete; }
...
PoliciesManipulator WorldLogic::Policies() { return {*this}; }

EventsManipulator WorldLogic::Events() { return {*this}; }

ConstEventsManipulator WorldLogic::Events() const { return {*this}; }

PoliciesDeck& WorldLogic::PoliciesDeck() { return _policyDeck; }

::GlobalEventsDeck& WorldLogic::EventsDeck() { return _eventsDeck; }

const ::GlobalEventsDeck& WorldLogic::EventsDeck() const { return _eventsDeck; }
...
WorldLogicRound WorldLogic::Round() { return {*this}; }

ConstWorldLogicRound WorldLogic::Round() const { return {*this}; }
...
void WorldLogic::BeginExecuteActions() { _actionExecutionState.Reset(*this); }

void WorldLogic::BeginProgressStage() {
    _progressState.Reset(*this);

    GetPlayers().ForEach(
        [](PlayerLogic& player) { player.BeginProgressStage(); });
}

void WorldLogic::EndProgressStage() {
    _progressState.Complete(*this);
    GetPlayers().ForEach(
        [](PlayerLogic& player) { player.EndProgressStage(); });
}
...
void WorldLogic::ActionExecutionState::Reset(WorldLogic& w) {
    ...
    w.GetPlayers().OrderedByDices().Apply(
        [&index, &players, this](PlayerLogic& p) {
...
bool WorldLogic::ActionExecutionState::IsComplete() const {
    return world().GetPlayers().All(
        [](const PlayerLogic& p) { return p.AreActionsDone(); });
}

std::tuple<bool, int, int, ActionType>
WorldLogic::ActionExecutionState::Next() {
    assert(!IsComplete());

    int count = 0;
    std::array<std::tuple<int, int, ActionType>, 4> possibleActions;

    for (int i = firstPlayer; i < firstPlayer + world().GetPlayersCount();
         ++i) {
        auto index = i % world().GetPlayersCount();

        auto& p = world().GetPlayer(index);
        if (p._actionExecutionState.IsComplete()) continue;
        auto [diceValue, actionType] = p._actionExecutionState.Current();
        possibleActions[count] = std::tuple(index, diceValue, actionType);
        count++;
    }

    std::sort(possibleActions.begin(), possibleActions.begin() + count,
              [](auto& l, auto& r) { return std::get<2>(l) < std::get<2>(r); });

    auto action = possibleActions.front();
    bool newAction = action != lastAction;
    lastAction = action;
    return std::tuple(newAction, std::get<0>(action), std::get<1>(action),
                      std::get<2>(action));
}
...
void WorldLogic::EndGame() {
    ...
    GetPlayers().OrderedByPoints().Apply([&index](PlayerLogic& logic) {
        INFO("Player {} took {} place with {} score points! Congratulations!!!",
             logic.GetId(), index++, logic.GetPoints());
    });
}
