...
vsg::ref_ptr<vsg::MatrixTransform> Clone(vsg::ref_ptr<vsg::MatrixTransform> original) {
    auto clone = vsg::MatrixTransform::create();
    clone->matrix = original->matrix;
    return clone;
}

vsg::ref_ptr<vsg::VertexIndexDraw> Clone(vsg::ref_ptr<vsg::VertexIndexDraw> original) {
    // consider immutable
    return original;
}
