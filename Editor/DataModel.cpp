...
DataModel::DataModel(boost::asio::io_context& ctx) : _ctx{ ctx } {
...
void DataModel::Execute(const ImportFileCommand& cmd) {
    ...
    boost::asio::post(_ctx, [this, cmd]() {
        auto importFile = [&]() {
            ...
            std::filesystem::path programPath = boost::dll::program_location().parent_path().native();
            std::filesystem::path requestedPath = cmd.FilePath;
            if (requestedPath.is_relative()) {
                requestedPath = programPath / requestedPath;
            }

            if (auto root = vsg::read_cast<vsg::Node>(requestedPath.native(), cmd.Options);
                root) {

                auto relPath = std::filesystem::relative(cmd.FilePath, programPath);

                Notify(IDataModelObserver::CompileCommand{
                    .Object = root, .OnComplete = [&](auto object, auto result) {} });
...
std::shared_ptr<AsyncQueue> DataModel::GetSyncContext() { return _queue; }
        ...
        try {
            ...
            if (auto it = o.find("packages"); it != o.end()) {
                for (auto entry : *it) {
                    ...
                    Execute(IDataModelEditor::ImportFileCommand{ .FilePath = path,
                                                                .Options = _options });
                }
            }
            else {
            ...
            Notify(LogNotification{ .Code = EDITOR_ERROR_PROJECT_FILE_IS_NOT_VALID,
...
void DataModel::Execute(const RenameLanguageCommand& cmd)
{
    _queue->Enqueue([=]() {
        ...
        if (auto it = std::find(langs.begin(), langs.end(), cmd.OldValue); it == langs.end()) {
            Notify(LogError(LOG_ENTRY_NOT_FOUND, cmd.OldValue));
            return;
        }

        if (auto it = std::find(langs.begin(), langs.end(), cmd.NewValue); it != langs.end()) {
        ...
        Notify(IDataModelObserver::LanguageRemoveNotification{ .Value = cmd.OldValue });
        Notify(IDataModelObserver::LanguageAddedNotification{ .Value = cmd.NewValue });
        });
}
...
void DataModel::Execute(const RequestSuggestedChildrenCommand& cmd)
{
    _queue->Enqueue([=]() {
        ...
        case EntryType::Group:
            ...
            // r.Suggestions.emplace_back(magic_enum::enum_name(EntryType::Material), magic_enum::enum_name(EntryType::Material));
