...
class SystemFonts
    : public ObservableBase<SystemFonts, ISystemFonts, IObservable> {
...
public:
  ...
  bool QueryInterface(const InterfaceId &id, void **o) override {
    if (id == GetIID<ISystemFonts>::Id) {
      *o = static_cast<ISystemFonts *>(this);
    } else if (id == GetIID<IObservable>::Id) {
      *o = static_cast<IObservable *>(this);
    } else if (id == GetIID<IReferenced>::Id) {
      *o = static_cast<IReferenced *>(static_cast<ISystemFonts *>(this));
    } else {
      *o = nullptr;
    }
  ...
  void Execute(const Refresh &cmd) override {
    ExecuteAsync([this, cmd]() {
      ...
      OnMessage(new FontsRefreshCompletedMessage{std::move(allFonts)});
    });
  }
  ...
  void Execute(const CompileFont &cmd) override {
    ExecuteAsync([this, cmd]() {
      ...
      if (auto it = _compiledFontsCache.find(cmd.DisplayName);
        ...
        OnMessage(new FontCompiledMessage{cmd.DisplayName, it->second.Font,
                                          it->second.Root, cmd.State});
        return;
      }

      if (auto it = this->_allFontFiles.find(cmd.DisplayName);
          it == this->_allFontFiles.end()) {
        OnError(new GenericError<std::string>{
            LOG_LEVEL_ERROR, LOG_ENTRY_NOT_FOUND, cmd.DisplayName});
        return;
        ...
        if (auto font = object.cast<vsg::Font>(); font) {
          ...
          if (row_length == 0) {
            OnError(new GenericError{LOG_LEVEL_ERROR, LOG_FILE_LOAD_FAILED,
                                     cmd.DisplayName});
            return;
          }
          ...
          OnMessage(new FontCompiledMessage{cmd.DisplayName, font, scenegraph,
                                            cmd.State});
        } else {
          OnError(new GenericError(LOG_LEVEL_ERROR, LOG_FILE_LOAD_FAILED,
                                   cmd.DisplayName));
