...
ViewerWindow::ViewerWindow(wxWindow *parent)
    : wxWindow(parent, wxID_ANY, wxDefaultPosition, wxSize(parent->GetSize())) {
...
void ViewerWindow::PaintNow() { Render(); }
...
void ViewerWindow::OnMouseMotion(wxMouseEvent &e) {
  ...
  window->bufferedEvents.push_back(
      vsg::MoveEvent::create(window, event_time, e.m_x, e.m_y, mask));
}
...
void ViewerWindow::OnMouseDown(wxMouseEvent &e) {
  ...
  window->bufferedEvents.push_back(vsg::ButtonPressEvent::create(
      window, event_time, e.m_x, e.m_y, mask, button));
}
...
void ViewerWindow::OnMouseUp(wxMouseEvent &e) {
  ...
  window->bufferedEvents.push_back(vsg::ButtonReleaseEvent::create(
      window, event_time, e.m_x, e.m_y, mask, button));
}
...
void ViewerWindow::OnMouseWheel(wxMouseEvent &event) {
  ...
  window->bufferedEvents.push_back(vsg::ScrollWheelEvent::create(
      window, event_time,
      event.GetWheelRotation() < 0 ? vsg::vec3(0.0f, -1.0f, 0.0f)
...
void ViewerWindow::OnSizing(wxSizeEvent &e) {
  ...
  window->bufferedEvents.push_back(vsg::ConfigureWindowEvent::create(
      window, event_time, e.GetRect().GetPosition().x,
      e.GetRect().GetPosition().y,
      static_cast<uint32_t>(e.GetSize().GetWidth()),
      static_cast<uint32_t>(e.GetSize().GetHeight())));
}

std::pair<vsg::ButtonMask, uint32_t>
ViewerWindow::ConvertMouseButtons(wxMouseEvent &e) const {
  uint16_t mask{0};
  uint32_t button = 0;
