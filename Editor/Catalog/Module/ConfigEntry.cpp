...
public:
  ...
  Result<bool> SetActiveLanguage(const std::string &value) override {
    ...
    if (auto it = std::find(_languages.begin(), _languages.end(), value);
        it == _languages.end()) {
      return CreateError(LOG_ENTRY_NOT_FOUND, value);
    }
...
IReferenced *CreateConfigEntry() {
  static_assert(
      std::is_base_of_v<IReferenced, Catalog::Private::ConfigEntry>);
  return static_cast<Catalog::IConfigEntry *>(
      new Catalog::Private::ConfigEntry());
}
