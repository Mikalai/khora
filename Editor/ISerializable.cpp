...
template <> struct GetIID<IJsonPackage> {
...
public:
  using Interfaces =
      QueryInterfaces<JsonPackage, IJsonPackage, IPackage, IPackageWriter,
                      IPackageReader, IPackageArrayWriter, IPackageArrayReader,
                      IReferenced>;
  ...
  Result<Ref<IPackageWriter>> Write(std::string_view name,
    ...
    if (auto j = value.Cast<IJsonPackage>(); j) {
      ...
      return {new GenericError(LOG_LEVEL_ERROR, LOG_NOT_IMPLEMENTED,
                               std::string("IPackage"))};
    }
  }
  ...
  Result<void> ReadFromFile(std::filesystem::path path) override {
    ...
    if (!stream.is_open()) {
      return {new GenericError{LOG_LEVEL_ERROR, EDITOR_ERROR_FILE_NOT_FOUND,
                               path.string()}};
    }
    ...
    try {
      ...
      return {new GenericError(LOG_LEVEL_ERROR,
  ...
  Result<std::int8_t> Int8At(std::int32_t index) const override {
    if (index < GetSize()) {
      ...
      return {new GenericError{LOG_LEVEL_ERROR, LOG_INDEX_OUT_OF_RANGE,
                               std::string{"Index"}}};
    }
  }
  ...
  Result<std::string> StringAt(std::int32_t index) override {
    if (index < GetSize()) {
      try {
        ...
        return {new GenericError{LOG_LEVEL_ERROR, LOG_FILE_LOAD_FAILED,
                                 std::string{e.what()}}};
      }
    } else {
      return {new GenericError{LOG_LEVEL_ERROR, LOG_INDEX_OUT_OF_RANGE,
  ...
  Result<Ref<IPackageWriter>> BeginPackage(std::string_view name) override {
    ...
    if (_currentWriter || _currentArrayWriter) {
      return {new GenericError{LOG_LEVEL_ERROR, LOG_OPERATION_NOT_SUPPORTED,
...
IReferenced *CreateJsonPackage() {
  ...
  return (IJsonPackage *)new JsonPackage();
}
