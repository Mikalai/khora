...
Ref<IConfigEntry> DataModelState::GetConfig() {
  return _dir->FindEntry({ROOT_CONFIG}).Cast<IConfigEntry>();
}

void DataModelState::SetDir(Ref<IDirectoryEntry> dir) {
  _dir = dir;
  assert(_dir);
...
Ref<ISystemFonts> DataModelState::GetFonts() { return _fonts; }
...
Ref<IDirectoryEntry> DataModelState::GetDir() { return _dir; }

Ref<IEntry> DataModelState::GetActiveEntry() { return _activeEntry; }

void DataModelState::SetActiveEntry(Ref<IEntry> entry) { _activeEntry = entry; }
