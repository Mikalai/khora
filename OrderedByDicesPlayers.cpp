...
OrderedByDicesPlayers::OrderedByDicesPlayers(WorldLogic& world)
	...
	std::sort(_players.begin(), _players.begin() + _world.GetPlayersCount(), [](auto l, auto r) {
		return std::get<1>(l) < std::get<1>(r);
		});
}

int OrderedByDicesPlayers::First() const {
	return std::get<0>(_players[0]);
}

void OrderedByDicesPlayers::Apply(std::function<void(PlayerLogic& player)> action) {
	for (int i = 0; i < _world.GetPlayersCount(); ++i) {
...
int OrderedByPointsPlayers::First() const {
	return std::get<0>(_players[0]);
}

void OrderedByPointsPlayers::Apply(std::function<void(PlayerLogic& player)> action) {
	for (int i = 0; i < _world.GetPlayersCount(); ++i) {
		auto& player = _world.GetPlayer(std::get<0>(_players[i]));
		action(player);
	}
