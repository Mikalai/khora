...
WorldState::WorldState(WorldLogic& logic) : _logic{logic} {}

WorldState* WorldState::Update() {
    if (_update) _update(_logic);

    for (auto& transition : transition_table) {
        if (transition.predicate(_logic)) {
            Exit();

            if (transition.next) transition.next->Enter();
...
void WorldState::Transition(std::function<bool(const WorldLogic&)> predicate,
                            WorldState* next) {
