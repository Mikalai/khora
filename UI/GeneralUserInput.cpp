...
GeneralUserInput::GeneralUserInput(int playerId, WorldLogic& world)
    : _world{world}, _playerId{playerId} {
    ...
    //{ 		self.PostWorldUpdateAction([&self, diceIndex, diceValue](WorldLogic&
    //logic) { 			logic.GetPlayer(self.PlayerId()).SetDiceValue(diceIndex,
    //diceValue);
    //			});
    //		});
    //	});
    ...
    /*Map().On([](GeneralUserInput& self, BeginSelectActionsMessage& msg) {
            self.SelectActionAsync([&self](int index, ActionType action) {
                    self.PostWorldUpdateAction([&self, index,
       action](WorldLogic& logic) {
                            logic.GetPlayer(self.PlayerId()).SetAction(index,
       action);
                            });
                    });
            });*/
...
const WorldLogic& GeneralUserInput::GetWorld() const { return _world; }

// void GeneralUserInput::ActivateExpedition(int expeditionId,
// SelectExpeditionCallback cb) { 	auto& player = _world.GetPlayer(PlayerId());
...
//     auto& player = _world.GetPlayer(PlayerId());
//     assert(std::find(player.GetDiscoveries().begin(),
//     player.GetDiscoveries().end(), discovery) !=
//     player.GetDiscoveries().end()); player.DropDiscovery(discovery);
// }
...
void GeneralUserInput::ExecuteActionAsync(ActionType action,
                                          std::function<void()> cb) {
    assert(action != ActionType::Unknown);

    INFO("GeneralUserInput[{}]: ExecuteActionAsync {}", PlayerId(),
         ActionToString[As<int>(action)]);
        ...
        case ActionType::Policy:
            PostWorldUpdateAction([this, cb](WorldLogic& world) {
                world.GetPlayer(PlayerId())
                    .BeginSelectPolicyFromHandsToActivate(
                        [this, cb, &world](PoliciesType policy) {
                            ...
                            if (p.CanActivatePolicy(policy)) {
                                ...
                                WARN(
                                    "Player {} can't activate policy {}. "
                                    "Submit another request.",
                                    PlayerId(), (int)policy);
                                PostWorldUpdateAction([this](
                                                          WorldLogic& world) {
                                    SelectPolicyFromHandsMessage msg{
                                        PlayerId(),
                                        PolicySelectionReasonType::Activate};
                                    auto v = msg;
                                    world.Notify(v);
                                });
                            }
...
// void GeneralUserInput::ActivatePolicy(Policies* policy, int count, int
// selected, SelectPolicyCallback cb) {
//     auto& player = _world.GetPlayer(PlayerId());
//     cb(player, policy, count, selected);
// }

bool GeneralUserInput::IsIdle() const {
    if (!std::all_of(_idleMarkers.begin(), _idleMarkers.end(),
                     [](auto& v) { return v->IsIdle(); })) {
        return false;
    }
...
void GeneralUserInput::AddIdleMarker(
    std::unique_ptr<IUserInputIdleMarker>&& marker) {
...
int GeneralUserInput::NextRandomDiceValue() { return diceDist(rng); }
