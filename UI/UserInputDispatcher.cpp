...
using UserInputDispatcherMapping =
    MessageMapping<UserInputDispatcher, ALL_MESSAGES>;
...
struct InitMapping {
    InitMapping(UserInputDispatcherMapping& m) {
        ...
        m.On(
            [](UserInputDispatcher& self, BeginSelectPlayersCountMessage& msg) {
                self.Handlers().ForEach([&msg](IUserInputHandler& h) {
        ...
        /*	m.On([](UserInputDispatcher& self, BeginCitySelectionMessage&
           msg) { self.Handlers().ForEach([&msg](IUserInputHandler& h) {
                                h.Next(msg);
                                });
                        });*/
        ...
        m.On([](UserInputDispatcher& self, ThrowDiceMessage& msg) {
            ...
            h.ThrowDiceAsync(msg.diceIndex, [&h](int diceIndex, int diceValue) {
                h.PostWorldUpdateAction(
                    [&h, diceIndex, diceValue](WorldLogic& logic) {
                        logic.GetPlayer(h.PlayerId())
                            .SetDiceValue(diceIndex, diceValue);
                    });
            });
        });
        ...
        /*m.On([](UserInputDispatcher& self, BeginSelectActionsMessage& msg) {
                self.Handlers().ForEach([&msg](IUserInputHandler& h) {
                        h.SelectActionAsync(msg.dices, [&h](int index,
           ActionType action) { h.PostWorldUpdateAction([&h, index,
           action](WorldLogic& logic) {
                                        logic.GetPlayer(h.PlayerId()).SetAction(index,
           action);
                                        });
                                });
                        });
        ...
        m.On([](UserInputDispatcher& self, SelectLawPolicyMessage& msg) {
            auto& h = self.Handlers().Select(msg.playerId);
            h.SelectLawPolicyAsync(
                msg.policyA, msg.policyB,
                [&h](PoliciesType selected, PoliciesType dropped) {
                    h.PostWorldUpdateAction([&h, selected,
                                             dropped](WorldLogic& world) {
                        INFO("Player {} selected policy {}, dropped policy {}",
                             h.PlayerId(),
                             GetPolicyTemplate(selected).GetName(),
                             GetPolicyTemplate(dropped).GetName());
        ...
        m.On([](UserInputDispatcher& self, SelectExpeditionMessage& msg) {
            ...
            h.SelectExpeditionAsync([&h, msg](int expedition) {
                h.PostWorldUpdateAction([&h, expedition,
                                         msg](WorldLogic& world) {
                    INFO("Player {} expedition selected {}", h.PlayerId(),
                         expedition);
                    PlayerLogic& p = world.GetPlayer(h.PlayerId());
        ...
        m.On([](UserInputDispatcher& self, BeginProgressMessage& msg) {
            ...
            h.MakeProgressAsync([&h, msg](ProgressTrackType track) {
                h.PostWorldUpdateAction([&h, track, msg](WorldLogic& world) {
                    INFO("Player {} advance track {}", h.PlayerId(),
                         (int)track);
                    PlayerLogic& p = world.GetPlayer(h.PlayerId());
                    ...
                    if (p.CanUpgradeTrack(track, 0, false)) {
                        ...
                        WARN(
                            "Player {} can't advance track {}. Submit another "
                            "request.",
                            h.PlayerId(), (int)track);
                        h.PostWorldUpdateAction([&h, msg](WorldLogic& world) {
                            auto v = msg;
                            world.Notify(v);
                        });
            ...
            h.MakeProgressAsync([&h, msg](ProgressTrackType track) {
                h.PostWorldUpdateAction([&h, track, msg](WorldLogic& world) {
                    ...
                    if (track == ProgressTrackType::Unknown) {
                        p.EndExtraProgress();
                    } else if ((msg.allowedTrack == ProgressTrackType::Any ||
                                msg.allowedTrack == track) &&
        ...
        m.On([](UserInputDispatcher& self, SelectPolicyFromHandsMessage& msg) {
            auto& h = self.Handlers().Select(msg.playerId);
            h.SelectPolicyFromHandsAsync(
                msg.reason, [&h, msg](PoliciesType policy) {
                    auto tag = rand();
                    INFO("{}: Player {} selected policy {} ({})", tag,
                         h.PlayerId(), ToString(policy), (int)policy);
                    h.PostWorldUpdateAction(
                        [&h, tag, policy, msg](WorldLogic& world) {
                            INFO("{}: Player {} selected policy {} ({})", tag,
                                 h.PlayerId(), ToString(policy), (int)policy);
                            PlayerLogic& p = world.GetPlayer(h.PlayerId());
        ...
        m.On([](UserInputDispatcher& self,
                SelectDiscoveryFromHandsMessage& msg) {
            auto& h = self.Handlers().Select(msg.playerId);
            h.SelectDiscoveryFromHandsAsync(
                [&h, msg](discovery_type discovery) {
                    h.PostWorldUpdateAction(
                        [&h, discovery, msg](WorldLogic& world) {
                            INFO("Player {} selected discovery {}",
                                 h.PlayerId(), (int)discovery);
                            PlayerLogic& p = world.GetPlayer(h.PlayerId());
        ...
        /*m.On([](UserInputDispatcher& self, BeginDropPolicyMessage& msg) {
                ...
                h.SelectPolicyFromHandsAsync([&h, msg](Policies policy) {
                        h.PostWorldUpdateAction([&h, policy, msg](WorldLogic&
        world) { INFO("Player {} selected policy {} to drop", h.PlayerId(),
        (int)policy); PlayerLogic& p = world.GetPlayer(h.PlayerId());
                                p.EndSelectPolicyFromHands(policy);
                        });
                });
        });		*/
        ...
        m.On([](UserInputDispatcher& self,
                SelectDiscoveryFromPoolMessage& msg) {
            auto& h = self.Handlers().Select(msg.playerId);
            h.SelectDiscoveryFromPoolAsync([&h, msg](discovery_type discovery) {
                h.PostWorldUpdateAction(
                    [&h, discovery, msg](WorldLogic& world) {
                        INFO("Player {} selected discovery {} ({}) to drop",
                             h.PlayerId(), ToString(discovery), (int)discovery);
                        PlayerLogic& p = world.GetPlayer(h.PlayerId());
        ...
        m.On([](UserInputDispatcher& self, SelectActionMessage& msg) {
            auto& h = self.Handlers().Select(msg.playerId);
            h.SelectActionAsync(
                msg.diceValue, msg.allowMilitary, [&h, msg](ActionType action) {
                    auto tag = rand();
                    INFO("{}: Player {} action select {} ({})", tag,
                         h.PlayerId(), ToString(action), (int)action);
                    h.PostWorldUpdateAction(
                        [&h, action, msg, tag](WorldLogic& world) {
                            INFO("{}: Player {} action select {} ({})", tag,
                                 h.PlayerId(), ToString(action), (int)action);
                            PlayerLogic& p = world.GetPlayer(h.PlayerId());
...
void UserInputDispatcher::Complete() { assert(false); }

void UserInputDispatcher::SelectPlayersCount() { assert(false); }

void UserInputDispatcher::SelectPlayersColor() { assert(false); }

void UserInputDispatcher::SelectCity() { assert(false); }
//
// void UserInputDispatcher::AdvanceAnyTrackForFree(int playerId) {
// assert(false); } void UserInputDispatcher::AdvanceAnyTrack(int playerId, int
// discont) { assert(false); } void
// UserInputDispatcher::AdvanceMilitaryTrack(int playerId, int discont) {
// assert(false); } void UserInputDispatcher::SelectArgosLevel2(int playerId) {
// assert(false); } void UserInputDispatcher::SelectThebesLevel2(int playerId) {
// assert(false); }
////void UserInputDispatcher::SelectExpedition(int playerId,
///SelectExpeditionCallback cb) { assert(false); }
// void UserInputDispatcher::PlayOstracism(int playerId) { assert(false); }
// void UserInputDispatcher::TakeDiscovery(int playerId, int count) {
// assert(false); } void UserInputDispatcher::DropPolicies(int playerId, int
// count) { assert(false); }
////void UserInputDispatcher::DropDiscovery(int playerId, DropDiscoveryCallback
///cb) { assert(false); }
// void UserInputDispatcher::TakePolicies(int playerId, int count) {
// assert(false); } void UserInputDispatcher::PlayPersiaCampaign() {
// assert(false); } void UserInputDispatcher::DoAnyActionExceptMilitary(int
// playerId) { assert(false); }
//
...
// void UserInputDispatcher::SelectPolicy(int playerId, Policies* a, int count,
// SelectPolicyCallback cb) { 	assert(false);
...
bool UserInputDispatcher::IsIdle() const {
    for (int i = 0; i < _playersCount; ++i) {
        if (!_handlers[i]->IsIdle()) return false;
    }
    return true;
}

AllHandler UserInputDispatcher::Handlers() { return AllHandler{*this}; }
...
UserInputDispatcher::UserInputDispatcher(WorldLogic& world)
    : _world{world},
      _aiHandlers{AIUserInput{0, world}, AIUserInput{1, world},
                  AIUserInput{2, world}, AIUserInput{3, world}} {}